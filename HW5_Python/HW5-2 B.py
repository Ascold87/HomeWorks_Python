...
import random
 # вариант с 1м игроком.
sweets_table = int(input("У нас есть такое количества конфет: "))
first_player = input("Имя сладкоежки: ")
second_player = "Я Есть Робот"
print('Имя второго -', second_player)
first_count = 0
second_count = 0
...
if turn == 2:
    print(f"Истинный сладкоежка - {first_player} - победитель !!! {second_player} - Обидно проиграл, но не остался голодным.")
else:
    print(f"Второй, якобы сладкоежка, который этот... - {second_player} - победитель !!! Живой игрок - {first_player} - остался голодным !!!")